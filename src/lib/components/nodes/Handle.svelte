<script lang="ts">
	...
	const validate = (c) => {
		...
		if (prev.data.context || next.data.context) {
			if (prev.data.context && !next.data.context) return true;
			else if (next.data.context && !prev.data.context) return true;
			else if (prev.data.context == next.data.context) return true;
		} else return true;
		return false;
	};
